...
export function DialogContent({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("max-h-[88vh] overflow-hidden rounded-md border border-[var(--border-color)] bg-[var(--card-bg)] p-4 text-[var(--foreground)] shadow-2xl", className)} {...props}>
      {children}
    </div>
  );
}
