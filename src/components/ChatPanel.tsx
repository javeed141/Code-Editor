...
export default function ChatPanel({ workspace }: { workspace: Workspace }) {
  ...
  // the current value without being recreated on every render.
  const workspaceRef = useRef(workspace);
  useEffect(() => {
    workspaceRef.current = workspace;
  }, [workspace]);

  // The transport is created once — reads workspace from the ref at request time.
  ...
  return (
    // ResizablePanel renders as flex-col min-h-0, so we use flex-1 not h-full
    <aside className="relative flex flex-1 min-h-0 flex-col overflow-hidden border-l border-[var(--border-color)] bg-[#12131a] p-2 max-[800px]:hidden select-none">
      {/* decorative blurs — behind all content */}
      ...
      <div className="relative z-10 flex h-9 shrink-0 flex-row items-center gap-2 p-1">
      ...
      <div className="relative z-10 min-h-0 flex-1 overflow-y-auto p-1 pt-3 space-y-3.5">
        {messages.length === 0 && (
        ...
        {isLoading && (
          <div className="mb-3.5 flex items-center gap-2" role="status" aria-live="polite">
            ...
            <div className="flex items-center gap-2 rounded-xl rounded-tl-sm bg-white/[0.06] px-3 py-2 text-[11px] text-slate-400">
              ...
              <span>{status === "submitted" ? "Thinking…" : "Streaming response…"}</span>
      ...
      <Separator className="shrink-0 mb-2 bg-white/10" />
